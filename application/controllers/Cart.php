...
class Cart extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $data['content']            = $this->cart->select([
            'cart.id', 'cart.qty', 'cart.subtotal', 'cart.weight',
            'product.title', 'product.image', 'product.price', 'product.weight', 'product.production_cost'
        ])
            ->join('product')
    ...
    public function add()
    {
        if (!$_POST || $this->input->post('qty') < 1) {
            ...
            $cart                   = $this->cart->where('id_user', $this->id)->where(
                'id_product',
                $input->id_product
            )->first();
